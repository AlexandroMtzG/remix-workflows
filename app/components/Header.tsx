...
export default function Header() {
  ...
  const [navigation] = useState([
    ...
    {
      name: "SaasRock",
      href: "https://saasrock.com/?ref=remix-workflows&utm_content=header",
      target: "_blank",
    },
  ]);
    ...
    <header className="bg-gray-50">
      <nav className="mx-auto flex max-w-7xl items-center justify-between p-6 lg:px-8" aria-label="Global">
        ...
        <div className="hidden lg:flex lg:gap-x-12">
          {navigation.map((item) => (
            <Link key={item.name} to={item.href} target={item.target} className="text-sm font-semibold leading-6 text-gray-900">
              {item.name}
            </Link>
          ))}
        </div>
      ...
      <Dialog as="div" className="lg:hidden" open={mobileMenuOpen} onClose={setMobileMenuOpen}>
        ...
        <Dialog.Panel className="fixed inset-y-0 right-0 z-10 w-full overflow-y-auto bg-white px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-gray-900/10">
          <div className="flex items-center justify-between">
            <Link to="/" className="-m-1.5 p-1.5">
              <span className="sr-only">Remix Workflows</span>
          ...
          <div className="mt-6 flow-root">
            <div className="-my-6 divide-y divide-gray-500/10">
              ...
              <div className="py-6">
                <a
                  href="https://github.com/AlexandroMtzG/remix-workflows"
